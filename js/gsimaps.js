...
CONFIG.DEM[0] = { type : "TXT", url : "//cyberjapandata.gsi.go.jp/xyz/dem5a/{z}/{x}/{y}.txt", z : 15, fixed : 1, src : "5m ( Airborne laser survey ) " };
CONFIG.DEM[1] = { type : "TXT", url : "//cyberjapandata.gsi.go.jp/xyz/dem5b/{z}/{x}/{y}.txt", z : 15, fixed : 1, src : "5m ( Photogrammetry ) " };
CONFIG.DEM[2] = { type : "TXT", url : "//cyberjapandata.gsi.go.jp/xyz/dem/{z}/{x}/{y}.txt"  , z : 14, fixed : 0, src : "10m"              };
...
GSI.TEXT.SAKUZU.DIALOG_SAVE_COMMENT2= 'Drawing information that consists of the "TEXT" and "marker (circle)" elements can be saved only in GeoJSON format.';
...
GSI.TEXT.SHARE.DIALOG_LINK_MESSAGE = 'You can share the map that is currently displayed by writing the following URL in your e-mail etc.'+'<div style="font-size:85%;">※ Drawing information is not reflected. Please share the GeoJSON/KML file(s) which you can save at Function > tools > Draw.</div>';
GSI.TEXT.SHARE.DIALOG_BUILTIN_MESSAGE = 'You can embed the GSI Maps in your website by adding the following tag to the HTML file.'+'<div style="font-size:85%;">※ Drawing information is not reflected. Please share the GeoJSON/KML file(s) which you can save at Function > tools > Draw.</div>';
GSI.TEXT.SHARE.DIALOG_SAVE_MESSAGE = 'Please save the following HTML by clicking the [Save the above HTML] button.'+'<div style="font-size:85%;">※Please use this HTML file as temporary one. If you want to share drawing information, please share the GeoJSON/KML file(s) which you can save at Function > tools > Draw.</div>';
...
GSI.MUNI_ARRAY["4423"] = '4,宮城県,4423,富谷町';
...
GSI.MUNI_ARRAY["9367"] = '9,栃木県,9367,岩舟町';
...
GSI.MeasureDialog = GSI.Dialog.extend( {
	...
	startAreaMeasure : function()
	{
		...
		L.drawLocal.draw.handlers.polygon.tooltip.cont = 'Click the next position.';
		L.drawLocal.draw.handlers.polygon.tooltip.end = 'Click the next position (Exit by clicking twice on the end point).';
...
GSI.ShareDialog = GSI.Dialog.extend( {
	...
	_createSettingContent : function()
	{
		...
		// layer
		item = __createItem( this,'Share layers in display.' );
		ul.append( item.li );
		...
		item = __createItem( this,'Open the "Layer" window.' );
		ul.append( item.li );
		...
		// Information list
		item = __createItem( this,'Open the "Layer list" window.' );
		ul.append( item.li );
		this._visibleLayerTreeDlgCheck = item.checkbox;

		// Share layer hierarchy
		item = __createItem( this,'Open the folder that consists viewing layers in the "Layer list" window.' );
		ul.append( item.li );
    ...
    , {
		...
		onMenuItemClick : function( id )
		{
			...
			// Measurement dialog
				...
				{
					alert( 'Can not measure during drawing.' );
					return;
				}
