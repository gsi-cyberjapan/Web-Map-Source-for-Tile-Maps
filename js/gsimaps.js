...
CONFIG.LAYERTYPELIST = {
	...
	"tile"          : { caption : "Tile", isTile: true, isTileImage : true },
...
// to use the gray scale for IE10,11 
//CONFIG.USEIE1011GRAYSCALE = true;
CONFIG.USEIE10GRAYSCALE = false;
CONFIG.USEIE11GRAYSCALE = true;

// Zoom level when you click on a search result
CONFIG.SEARCHRESULTCLICKZOOM = 15;

// Number of markers of search results (all -1)
CONFIG.SEARCHRESULTMAXMARKERNUM = -1;

// Style of latitude and longitude grid
...
// Storing the hierarchy of the layer tree
...
// The number of magnetic north line
CONFIG.JIHOKULINECOUNT = 3;

// Style of magnetic north line
...
// Threshold that magnetic north lines are displayed
CONFIG.JIHOKULINEAVAILABLEZOOM = 11;

// Print paper size
CONFIG.PAPERSIZE = {
	"A4_portrait" : {w:650,h:900},  //A4Longitudinal
	"A4_landscape" : {w:950,h:550}, //A4side
	"A3_portrait" : {w:950,h:1350},  //A3Longitudinal
...
// Time of the double click judgment
CONFIG.DBLCLICKINTERVAL = 300; // millisecond

// Right double-click decision millisecond
...
/************************************************************************
 Setting: Grid (zoom level correspondence)
 ************************************************************************/
...
CONFIG.UTMGRID = {
	CONDITION : [
		{ zoom : 7, grid : 'a'},          //  7 or less Wide area
		{ zoom : 10, grid : 100 * 1000 }, // less than 10 100km
...
GSI.TEXT.SAKUZU.LOAD_NOFILE = 'File has not been selected.';

GSI.TEXT.SAKUZU.DIALOG_TITLE = 'Draw';
GSI.TEXT.SAKUZU.DIALOG_TOOLTIP_LOAD = 'Open a file';
GSI.TEXT.SAKUZU.DIALOG_TOOLTIP_SAVE = 'Save the information';
GSI.TEXT.SAKUZU.DIALOG_TOOLTIP_ADDMARKER = 'Add marker (icon)';
...
GSI.TEXT.SAKUZU.DIALOG_LOAD_CANCELBTN = 'Cancel';
GSI.TEXT.SAKUZU.DIALOG_LOAD_NOFILE = 'File has not been selected.';
GSI.TEXT.SAKUZU.DIALOG_LOAD_NOTEXT = 'Text has not been entered.';
GSI.TEXT.SAKUZU.DIALOG_LOAD_ERROR = 'Failed to load, please check the format of the file.';

GSI.TEXT.SAKUZU.DIALOG_SAVE_COMMENT = 'Select the file format.';
GSI.TEXT.SAKUZU.DIALOG_SAVE_COMMENT2= 'What was constructed in the "TEXT" and "marker (circle)" it can only be saved in GeoJSON format.';
...
GSI.TEXT.SAKUZU.DIALOG_EDIT_REMOVELAYERCONFIRMMSG = 'You want to delete this layer. Is it OK?';
...
GSI.TEXT.SHARE.DIALOG_LINK_TITLE = 'Get the link';
GSI.TEXT.SHARE.DIALOG_BUILTIN_TITLE = 'Embedded in the web site';
GSI.TEXT.SHARE.DIALOG_SAVE_TITLE = 'Save as a new file';

GSI.TEXT.SHARE.DIALOG_LINK_MESSAGE = 'By sending the following URL in your e-mail, you can share the map that is currently displayed.'+'<div style="font-size:85%;">※Plotting the results are not reflected. Please share to save the drawing results using the drawing file function in the file.</div>';
GSI.TEXT.SHARE.DIALOG_BUILTIN_MESSAGE = 'By adding the following tag to the HTML file, you can embed the GSI Maps to the web site.'+'<div style="font-size:85%;">※Plotting the results are not reflected. Please share to save the drawing results using the drawing file function in the file.</div>';
GSI.TEXT.SHARE.DIALOG_SAVE_MESSAGE = 'The following HTML please save by clicking the [Save the above HTML] of the button.'+'<div style="font-size:85%;">※HTML, please use as temporary. Drawing information, it is recommended that you save to a file using the drawing function.</div>';
GSI.TEXT.SHARE.DIALOG_SAVE_MESSAGE_IE8 = 'Please save <strong>the character code UTF-8</strong> the following HTML in a text editor.' ;

GSI.TEXT.SHARE.DIALOG_TEMPLATELOADERROR = 'Sorry, please use at a later time.' ;

GSI.TEXT.SHARE.DIALOG_DOWNLOADBTN = 'Save the above HTML';
...
// ie8 array map, filter method additional
try
{
(function(fn){
...
// Leave it to jQuery for setOpacity of the leaflet does not work in IE8
L.DomUtil.setOpacity = function(el,opacity)
{
	$(el).css({opacity:opacity} );
};

// In the case of JQuery.ajax IE11 order to use the XMLHttpRequest
...
Proj4js.defs['EPSG:4301'] = "+proj=longlat +ellps=bessel +towgs84=-146.336,506.832,680.254,0,0,0,0 +no_defs";	//the old Japan Geodetic System (longitude and latitude coordinates)
...
GSI.Draw.CircleMarker = L.Draw.Circle.extend( {
	_onMouseMove: function (e)
	{
		...
		if (this._isDrawing) {
			...
			this._tooltip.updateContent({
				text: this._endLabelText,
				subtext: showRadius ? 'radius: ' + GSI.Utils.ConverUnit(GSI.GLOBALS.map, this._shape, radius, "m", "px") + 'px': ''
			});
		}
...
GSI.Draw.FreehandPolyline = L.Draw.SimpleShape.extend({
	...
	initialize: function (map, options) {
		...
		this._initialLabelText = "Start drawing lines with the mouse down";
...
GSI.showTopMassage = function(){
    var message = ( CONFIG.TOPMESSAGE && CONFIG.TOPMESSAGE.DETAILS ? CONFIG.TOPMESSAGE.DETAILS : 'There is no current information.' );
...
GSI.Utils.getTimeStampString = function() {
	...
	var msec = now.getMilliseconds(); // Millisecond
	var result =
		year + '' +
		( '00' + month ).slice(-2)  +
		( '00' + day ).slice(-2) +
...
{
	//longitude：122～154degree
	//latitude：20～46degree
	return !( latLng.lat < 20 || latLng.lat > 46 || latLng.lng <122 || latLng.lng >154 );
}

// To calculate the polarization angle of the specified latitude and longitude, it returns the angle.
GSI.Utils.getVariation = function(latLng)
{
	var px = latLng.lng;
	var py = latLng.lat;

	//Round longitude and latitude coordinates (decimal) to six decimal places.
	px = px * 1000000;
	px = parseInt(px);
	px = px / 1000000;
	py = py * 1000000;
...
GSI.LayerTreeDialog = GSI.Dialog.extend( {
	...
	createContent : function()
	{
		...
		var li = $( '<li>' ).addClass( 'nodata' ).html( 'Loading' );
	...
	_createControl : function()
	{
		...
		this._showAllButton = $( '<a>' ).attr( { href:'javascript:void(0);'} ).html( 'Select all'   ).addClass( 'normalbutton showallbutton' );
		this._hideAllButton = $( '<a>' ).attr( { href:'javascript:void(0);'} ).html( 'All deselect' ).addClass( 'normalbutton showallbutton' );
	...
	_makeToolTip : function( item )
	{
		...
		{
			legend =$( '<a>' ).html( 'Show Legend' ).addClass( 'legend' ).attr( { 'href' : item.legendUrl, 'target' : '_blank' } );
		}
		if ( legend )
	...
	_makeLayer : function( li, a, item )
	{
		...
		if ( CONFIG.VISIBLELAYERTYPE )
		{
			...
			if ( item.cocotile )
			{
				var span = $( '<span>' ).addClass( 'cocotile' ).html('cocotile' );
				info.append( span );
			}
...
/************************************************************************
 L.Class
 - GSI.Dialog
   - GSI.MeasureDialog (Measurement dialog management)
 ************************************************************************/
GSI.MeasureDialog = GSI.Dialog.extend( {
	...
	startDistanceMeasure : function()
	{
		...
		L.drawLocal.draw.handlers.polyline.tooltip.cont = 'Select the following position(Exit by clicking on the final point twice)';
		L.drawLocal.draw.handlers.polyline.tooltip.end = 'Select the following position(Exit by clicking on the final point twice)';
	...
	startAreaMeasure : function()
	{
		...
		L.drawLocal.draw.handlers.polygon.tooltip.start = 'Select the starting position';
		L.drawLocal.draw.handlers.polygon.tooltip.cont = 'Select the following position';
		L.drawLocal.draw.handlers.polygon.tooltip.end = 'Select the following position(Exit by clicking on the final point twice)';
	...
	onLayerClick : function(layer, e)
	{
		...
		if ( isPolygon )
		{
			...
			this.distance.html( 'Area:'+ areaStr );
		}
		else
		{
			//Distance
			...
			this.distance.html( 'Distance:' + distanceStr);
		}
		return true;
	},
	startFeatureMeasure : function()
	{
		...
		this.map.eachLayer(
			L.bind( function(layer){
				...
				// Ignore anything that is not a polygon and line
...
/************************************************************************
 L.Class
 - GSI.Dialog
   - GSI.SakuzuDialog (Drawing dialog management)
 ************************************************************************/
GSI.SakuzuDialog = GSI.Dialog.extend( {
	...
	_createFileSavePanel : function()
	{
		...
		var label = $( '<label>' ).attr( {'for': id} ).html( 'KML format' );
		...
		var label = $( '<label>' ).attr( {'for': id} ).html( 'GeoJSON format' );
		selectFrame.append( radio );
		selectFrame.append( label );
	...
	_createPointEditFrame : function()
	{
		...
		var tr = $( '<tr>' );
		tr.append( $( '<td>' ).css( {'white-space':'nowrap'} ).html( 'icon:' ) );
		tr.append( $( '<td>' ).append( this._pointIconImage ) );
		tr.append( $( '<td>' ).css( {'white-space':'nowrap'} ).html( '&nbsp;&nbsp;rate of magnification:' ) );
	...
	_createLineEditFrame : function()
	{
		...
		tr.append( $( '<td>' ).css({"white-space":"nowrap"}).html( 'Line width:' ) );
		...
		tr.append( $( '<td>' ).css({"white-space":"nowrap"}).html( '&nbsp;&nbsp;Line color:' ) );
		...
		this._lineOpacityTextArea.text('Transparency of the line:0%');
		...
		var sliderChangeHandler = L.bind( function(opacitySlider) {
			...
			this._lineOpacityTextArea.text('Transparency of the line:'+opacityPercentage+'%');
	...
	_createFillEditFrame : function()
	{
		...
		tr.append( $( '<td>' ).css({"white-space":"nowrap"}).html( 'Fill color:' ) );
		...
		this._fillOpacityTextArea.text('Transparency of fill:0%');
		...
		var sliderChangeHandler = L.bind( function(opacitySlider) {
			...
			var opacityPercentage = opacity;
			this._fillOpacityTextArea.text('Transparency of fill:'+opacityPercentage+'%');
			opacity = 1 - ( opacity / 100 );
	...
	_createCircleEditFrame : function()
	{
		...
		td = $( '<td>' ).html('radius:').css( { "white-space":"nowrap"} );
		tr.append( td );
	...
	_createInfoEditFrame : function()
	{
		...
		tr.append( $( '<td>' ).css( { 'white-space':'nowrap'} ).html('name:') );
		var td = $( '<td>' );

		this._titleInput = $( '<input>' ).addClass( 'inputtext').attr({'type':'text', 'placeholder':'(e.g.:A library)'}).css( { width:'200px'} );
	...
	_createSelectEditTargetPanel : function(item)
	{
		if ( !this._selectEditTargetPanel )
		{
			...
			var message = $( '<div>' ).html( 'Select the object you edit.').addClass('message');
			var btnFrame = $( '<div>' ).addClass( 'gsi_sakuzu_dialog_okcancel' );
			var cancelBtn = $( '<a>' ).attr( {'href':'javascript:void(0);'} ).html( 'End the editing' ).addClass( 'normalbutton' );
	...
	_createEditInfoTableLine : function( no, key, value )
	{
		...
		var nameTextarea = $( '<textarea>' ).addClass( 'inputtextarea')
			.attr( { name: 'info_table_key', 'placeholder':'(e.g.:address)', rows:1} )
		...
		var valueTextarea = $( '<textarea>' ).addClass( 'inputtextarea')
			...
			.attr( { name: 'info_table_value', 'placeholder':'(e.g.:Tokyo)',rows:1} ).val( value ? value : '' );
		...
		td = $( '<td>' ).css( { width:"24px","text-align":"center"} );
		var btn = $( '<a>' ).attr( { 'href' : 'javascript:void(0);'} ).addClass( "btn" )
			.html( '<img title="Add a line to the lower" src="image/sakuzu/icon_enter.png">' )
			.click( L.bind(
				function(tr){
	...
	_initEditInfo : function( info )
	{
		...
		// header
		var tr = $( '<tr>' );
		var td = $( '<td>' ).css( { 'width' : '30%', 'white-space':'nowrap'} ).html( 'item name' );
		tr.append( td );
		td = $( '<td>' ).attr( { "colspan":"3"} ).html( 'value' );
		tr.append( td );
...
GSI.SearchResultDialog = GSI.Dialog.extend( {
	...
	createContent : function()
	{
		...
		this.shiSelect.append( $('<option>').html("Municipality").val("," ) );
		...
		var li = $( '<li>' ).addClass( 'nodata' ).html( 'out of' );
		this.listContainer.append( li );
		this.listFrame.append( this.listContainer );
	...
	initializeKenSelect : function()
	{
		...
		select.empty();
		select.append( $('<option>').html("Prefectures").val("," ) );
	...
	onKenChange : function()
	{
		...
		select.empty();
		select.append( $('<option>').html("Municipality").val("," ) );
	...
	clear : function()
	{
		...
		ul.empty();
		var li = $( '<li>' ).addClass( 'nodata' ).html( 'searching' );
		this.listContainer.append( li );
	...
	showResult : function()
	{
				...
				if (addressCode) {
					...
					$.ajax({
						...
						success : function(data2){
							// Display the results of the reverse geocoder to screen.
							if (data2.results){
		...
		this.setTitle( 'search results:' + viewNum + 'matter of ' + num + 'matter display' );
...
/************************************************************************
 L.Class
 - GSI.Dialog
   - GSI.ShareDialog（Share dialog management）
 ************************************************************************/
GSI.ShareDialog = GSI.Dialog.extend( {
	...
	_createSettingContent : function()
	{
		...
		this._settingContent.append( $( '<h3>' ).html( 'Basic configuration' ) );
		...
		// longitude latitude
		item = __createItem( this,'Share the center position and zoom in display' );
		ul.append( item.li );
		this._positionShareCheck = item.checkbox;

		// background map
		item = __createItem( this,'Share the background map in the display' );
		ul.append( item.li );
		this._basemapCheck = item.checkbox;

		// layer
		item = __createItem( this,'Share the layer in display' );
		ul.append( item.li );
		this._layerpCheck = item.checkbox;

		this._settingContent.append( ul );

		this._settingContent.append( $( '<h3>' ).html( 'Function settings' ) );

		ul = $( '<ul>' );

		// Display the search and Announcements bar
		item = __createItem( this,'Display the search and Announcements bar' );
		ul.append( item.li );
		this._visibleHeaderCheck = item.checkbox;

		// Display the information button
		item = __createItem( this,'Display the information button' );
		ul.append( item.li );
		this._visibleInfoMenuCheck = item.checkbox;

		// Display the function button
		item = __createItem( this,'Display the function button' );
		ul.append( item.li );
		this._visibleFuncMenuCheck = item.checkbox;

		// Display the context menu
		item = __createItem( this,'Display the context menu' );
		ul.append( item.li );
		this._visibleContextMenuCheck = item.checkbox;

		this._settingContent.append( ul );

		this._settingContent.append( $( '<h3>' ).html( 'Information set in the selection' ) );

		ul = $( '<ul>' );

		// Information in the selection
		item = __createItem( this,'Opening the information in the selected' );
		ul.append( item.li );
		this._visibleViewListDlgCheck = item.checkbox;

		this._settingContent.append( ul );

		this._settingContent.append( $( '<h3>' ).html( 'Information list setting' ) );

		ul = $( '<ul>' );

		// Information list
		item = __createItem( this,'Opening the information list' );
		ul.append( item.li );
		this._visibleLayerTreeDlgCheck = item.checkbox;

		// Share layer hierarchy
		item = __createItem( this,'Display in a state in which the open folder in the display' );
		ul.append( item.li );
		this._showCurrentFolderCheck = item.checkbox;

		this._settingContent.append( ul );

		this._settingContent.append( $( '<h3>' ).html( 'Setting display' ) );

		ul = $( '<ul>' );

		// Center crosshairs
		item = __createItem( this,'Display the center crosshairs' );
		ul.append( item.li );
		this._centerCrossCheck = item.checkbox;

		// Latitude and longitude grid
		item = __createItem( this,'Display latitude and longitude grid' );
		ul.append( item.li );
...
/************************************************************************
 L.Class
 - GSI.Dialog
   - GSI.ViewListDialog（Showing dialog management）
 ************************************************************************/
GSI.ViewListDialog = GSI.Dialog.extend( {
	...
	_makeLayer : function( li, a, item, isTile )
	{
		...
		if (! item._visibleInfo._isHidden )
		{
			viewMark.addClass( 'viewmark' ).html( 'display' );
			a.addClass( 'view' );
		}
		else
		{
    ...
    {
        var v = parseInt(opacity.text().replace("transparency", "").replace("%", ""), 10);
        if(c == "++"){
            v++;
        }
        else if(c == "+"){
...
/************************************************************************
 L.Class
 - GSI.GeoLocation（Position information acquisition）
 ************************************************************************/
...
GSI.HashOptions = L.Class.extend( {
    ...
    HashCreateProc : function(){
        ...
        if(this.eHashChange){
            ...
            // Basic configuration：Share the background map in the display
            // base=
            ...
            // Basic configuration：Share the layer in display
            // ls=
            ...
            // Information list setting：Share layer hierarchy
            // lcd=
		    ...
		    var visibles = {};
		    visibles[ CONFIG.PARAMETERNAMES.CENTERCROSS] = GSI.GLOBALS.pageStateManager.getViewSettingVisible(CONFIG.PARAMETERNAMES.CENTERCROSS); // Setting display：Center crosshairs        vs=c[0/1]
		    visibles[ CONFIG.PARAMETERNAMES.JIHOKULINE ] = GSI.GLOBALS.pageStateManager.getViewSettingVisible(CONFIG.PARAMETERNAMES.JIHOKULINE ); // Setting display：Latitude and longitude grid  vs=l[0/1]
		    visibles[ CONFIG.PARAMETERNAMES.LATLNGGRID ] = GSI.GLOBALS.pageStateManager.getViewSettingVisible(CONFIG.PARAMETERNAMES.LATLNGGRID ); // Setting display：UTM grid       vs=u[0/1]
		    visibles[ CONFIG.PARAMETERNAMES.UTMGRID    ] = GSI.GLOBALS.pageStateManager.getViewSettingVisible(CONFIG.PARAMETERNAMES.UTMGRID    ); // Setting display：Magnetic north line            vs=j[0/1]
		    visibles[ CONFIG.PARAMETERNAMES.FOOTER     ] = GSI.GLOBALS.footer.isVisible();
		    var visibleDialogs = {};
...
GSI.JihokuLine = L.Class.extend( {
	...
	{
		var center = this._map.getCenter();
		//Hide or Following out-of-range
		//longitude：122 degree～154 degree
		//latitude：20 degree～46 degree
		if (
		...
		// If the zoom level is below the set, do not show.
		...
		var pi = Math.PI;
		var center = this._map.getCenter();
		var rad = variation * pi / 180;	// Convert the angle in radians
		
		// Longitude of the center of the map
		var centerLng = this._map.getCenter().lng;
		var bounds = this._map.getBounds();

		// The height of the area being displayed
		var mapHeight = Math.abs(bounds.getSouth() - bounds.getNorth());
		// The width of the area being displayed
...
/************************************************************************
 L.Class
 - GSI.LatLngGrid（Latitude and longitude grid）
 ************************************************************************/
GSI.LatLngGrid = L.Class.extend( {
	...
	refresh : function()
	{
		...
		// Delete unnecessary lines and labels
...
/************************************************************************
 L.Class
 - GSI.LayerOpacitySetter（Opacity settings to each layer）
 ************************************************************************/
...
/************************************************************************
 L.Class
 - GSI.LayersJSON（layers.txt load）
 ************************************************************************/
...
/************************************************************************
 L.Class
 - GSI.MapLayerList（Display layer management）
 ************************************************************************/
GSI.MapLayerList = L.Class.extend( {
	...
	append : function( info, noFinishMove, isHide ,Confirm_FLAG)
	{
		...
		// 		var KARI=this;
		// 		jConfirm("航空法第１３２条で規定する無人航空機の飛行禁止空域のうち、航空法施行規則第２３６条第１号に掲げる空域（空港等の周辺空域）の投影面下となる場所を表示します。<br>なお、この情報には誤差が含まれている場合がありますので、境界付近等正確な空域については空港等の管理者に確認願います。<br>詳細については、<a target='_blank' href='http://www.mlit.go.jp/koku/koku_tk10_000003.html'>国土交通省ホームページ</a>で確認してください。", 'Consideration', function(r) {
		// 			if(r) {
	...
	_showLoading : function(title)
	{
		if ( !this._showLoadingInc || this._showLoadingInc == 0 )
		{
			...
			GSI.Modal.LoadingMessage.show( 'Loading files...' );
		}
...
/************************************************************************
 L.Class
 - GSI.MapMenu（Maps、Function menu）
 ************************************************************************/
GSI.MapMenuList = [];
...
/************************************************************************
 L.Class
 - GSI.MapMouse（Mouse operation control on the map）
 ************************************************************************/
...
GSI.Modal.Dialog = GSI.Modal.BaseClass.extend( {
	options : {
		positiveButtonText : 'Decision',
		nagativeButtonText : 'Stop',
		blindClose : false,
...
/************************************************************************
 ...
 - GSI.Modal.BaseClass
   - GSI.Modal.Dialog
     - GSI.Modal.FileSelectDialog（File selection dialog）
 ************************************************************************/
GSI.Modal.FileSelectDialog = GSI.Modal.Dialog.extend( {
	...
	getContent : function()
	{
		...
		if ( GSI.Utils.hasFileAPI )
		{
			...
			this.fileMessage = $( '<div>' ).addClass( 'message' ).html( 'After you select a file, click the Decision button' );
		...
		this.sourceMessage = $( '<div>' ).addClass( 'message' ).html( 'After entering the source, click the Decision button' );
	...
	onPositiveButtonClick : function()
	{
		...
		else
		{
			...
			{
				alert( 'The source has not been entered.' );
			}
		}
	},
	onNegativeButtonClick : function()
...
/************************************************************************
 L.Class
 - GSI.Modal.BaseClass
   - GSI.Modal.LoadingMessage（Loading message）
 ************************************************************************/
...
GSI.PagePrinter = L.Class.extend( {
	...
	_create : function()
	{
		...
		this._paperSizeSelect.append( $('<option>').html("A4 portrait (standard)").val("A4_portrait" ) );
		this._paperSizeSelect.append( $('<option>').html("A4 portrait (high resolution)").val("A4_portrait,hq" ) );
		this._paperSizeSelect.append( $('<option>').html("A4 landscape (standard)").val("A4_landscape" ) );
		this._paperSizeSelect.append( $('<option>').html("A4 landscape (high resolution)").val("A4_landscape,hq" ) );
		this._paperSizeSelect.append( $('<option>').html("A3 portrait (standard)").val("A3_portrait" ) );
		this._paperSizeSelect.append( $('<option>').html("A3 portrait (high resolution)").val("A3_portrait,hq" ) );
		this._paperSizeSelect.append( $('<option>').html("A3 landscape (standard)").val("A3_landscape" ) );
		this._paperSizeSelect.append( $('<option>').html("A3 landscape (high resolution)").val("A3_landscape,hq" ) );
		this._paperSizeSelect[0].selectedIndex = 0;
		this._paperSizeSelect.change( L.bind( this._paperSizeChage, this ) );
		td.append( $( '<span>' ).html( 'Paper size：' ).addClass( 'no_print' ));
		td.append( this._paperSizeSelect );
		tr.append( td );

		// Print button
		td = $( '<td>' ).css( {width:"64px"} );
		var printBtn  = $( '<button>' ).html( 'Print' ).addClass( 'no_print' ).click( L.bind( this.print, this ) );
		td.append( printBtn );
		tr.append( td );
...
　GET Parameters||hashll Decimal longitude,Decimal longitude
	z    Zoom level
	base base
	hc   Easter egg
	vs   Display set of values
	ls   Layer to be displayed
	skz  Drawing layer
************************************************************************/
...
/************************************************************************
 L.Class
 - GSI.SakuzuList（Drawing list management）
 ************************************************************************/
GSI.SakuzuListItem = L.Class.extend( {
	...
	{
		// Start the point editing
		L.drawLocal.draw.handlers.marker.tooltip.start = 'Click the point to put the marker';
	...
	{
		// Start point (text) editing
		L.drawLocal.draw.handlers.marker.tooltip.start = 'Click the point to put the text';
...
GSI.Searcher = L.Class.extend( {
	...
	checkQuery : function( q )
	{
		...
		else if ( q.match(/^[0-9]+°[\s]+[0-9]+°$/)
			||
			 q.match(/^[0-9]+°[0-9]+′[\s]+[0-9]+°[0-9]+′$/)
			||
			 q.match(/^[0-9]+°[0-9]+′[0-9]+(\.[0-9]+)*″[\s]+[0-9]+°[0-9]+′[0-9]+(\.[0-9]+)*″$/)
		)
		{
	...
	parseLatLngText2 : function( s )
	{
		...
		var matchArr =  s.match( /^([0-9]+)°[\s]+([0-9]+)°$/ );
		...
		matchArr = s.match(/^([0-9]+)°([0-9]+)′[\s]+([0-9]+)°([0-9]+)′$/);
		...
		matchArr = s.match(/^([0-9]+)°([0-9]+)′([0-9]+)″[\s]+([0-9]+)°([0-9]+)′([0-9]+)″$/);
...
/************************************************************************
 L.Marker
 - GSI.CenterCrossMarker（Center marks）
 ************************************************************************/
    ...
    , {
		...
		onMenuItemClick : function( id )
		{
			...
			// Measurement dialog
				...
				{
					alert( 'Can not measure during plotting' );
					return;
				}
