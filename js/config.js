...
(function($){
	$.fn.menu = function(options)
	{
		...
		this.find( '*[menu_method=new]' ).click(
            function(){
    			if(confirm( 'Information currently being edited will be discarded. Is it OK?' )){
				    o.oTree.tree('reset', o, null, "layers.txt");
			    }
		    }
        );

		// Dialog: Open the file
...
(function($){
	$.fn.loadDialog = function(options){
		...
		container.find("*[load_method=load]").click(
            function(){
			    if(_fileCheck()){
				    ...
				    alert( 'Please select a file with the extension .txt .json' );
			    }
		    }
        );
		...
		var _load = function(file){
			...
			reader.onerror = function(){ alert( 'Can not open the file' ); };
		...
		var _onFileLoad = function(){
			...
			try{
				...
				alert( 'an error in the description of the file' );
			}
		...
		// Converted into an internal format
...
(function($){
	...
	var _createTreeItem = function(o, treeObject, item){
		...
		var titleText = (item.title && item.title != '' ? '<span style="color:#222;">' +item.title + '</span>' : '<span style="color:#999;">No name</span>' );
            ...
            function() {
			    ...
			    a.attr( { href:'javascript:void(0);', target:''} ).click(
                    function(){
                        ...
                        if(o.fSaveMsg){
                            f = false;
                            if(window.confirm("Elements in the folder where the layer definition file URL has been entered will not be saved.")){
                                f = true;
                            }
                        }
                        if(f){
