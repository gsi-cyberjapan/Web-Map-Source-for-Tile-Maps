...
GSI.TEXT.SAKUZU.DIALOG_LOAD_COMMENT = 'Select <strong>KML, GeoJSON</strong> file<br><div style="font-size:85%">※You can also load files by dragging and dropping them on the map.</div>';
GSI.TEXT.SAKUZU.DIALOG_LOAD_COMMENT_IE8 = '<strong>KML,GeoJSON</strong>Enter the contents of the file.<br><div style="font-size:85%">※You can also load files by dragging and dropping them on the map</div>' ;
GSI.TEXT.SAKUZU.DIALOG_LOAD_FILENAMECAPTION = 'Display name on the panel.' ;

GSI.TEXT.SAKUZU.DIALOG_LOAD_OKBTN = 'Start reading';
GSI.TEXT.SAKUZU.DIALOG_LOAD_CANCELBTN = 'Cancel';
GSI.TEXT.SAKUZU.DIALOG_LOAD_NOFILE = 'File is not selected.';
...
GLOBE.MENU.CAMERARESET = {
	...
	create: function()
	{
		this.container = $('<div>Reset<br>the View</div>')
			.addClass('menu_btn')
			.attr({
				'id': 'camera_reset',
				'title': 'To the default viewpoint position'
			})
			.on('click', this.onClick.bind(this))
			.appendTo(document.body);
	},
...
GLOBE.DIALOG.FILEREAD = $.extend({}, new GLOBE.CLASS.DIALOG('gsi_dialog_fileread'), {
	...
	_refreshList : function()
	{
		...
		for ( var i=0; i<this._list.length; i++ )
		{
			...
			var clearBtn = $( '<a>' ).attr( {"href":"javascript:void(0);"} ).html( 'delete' ).addClass(buttonClassName);
			...
			label = $( '<label>' ).attr( {'for': id} ).html( 'Show labels' );
	...
	_onFileLoadOkClick : function()
	{
		...
		else
		{
			...
			if ( text != '' )
			{
				...
				{
					alert("Input in KML or GeoJSON format.");
				}
			}
			else
			{
	...
	{
		if ( confirm('You want to delete this layer. Is it OK?') )
		{
...
GLOBE.DIALOG.HEIGHTPOWER = $.extend({}, new GLOBE.CLASS.DIALOG('gsi_dialog_heightpower'), {
	...
	create: function()
	{
		...
		this.setDialogHeader('Height magnification');
	...
	_initializeContent: function()
	{
		...
		this.label = $('<div>Magnification of the height direction：</div>')
			.css({
				'margin': '20px',
				'font-size': '14px',
				'text-align': 'center'
...
GLOBE.DIALOG.GETLINK = $.extend({}, new GLOBE.CLASS.DIALOG('gsi_dialog_getlink'), {
	...
	{
		this.setDialogHeader('Get the link');
	},
	...
	_setMessage : function()
	{
		...
		var msg = "You can share the map that is currently displayed by writing the following URL in your e-mail etc.";
...
GLOBE.DIALOG.SEARCH = $.extend({}, new GLOBE.CLASS.DIALOG('gsi_dialog_search'), {
	...
	showResult : function()
	{
		...
		this.setTitle( 'search results:' + num + 'matter of' + viewNum + 'matter display' );
		this.show();
	},
...
GLOBE.MAP = {
	...
	setClickEvent : function ()
	{
		...
		leftClickHandler.setInputAction(function(event){
			...
			if ( obj && obj.primitive && obj.primitive.data && obj.primitive.data.type == 'SEARCH' )
			{
				...
				title = data.title;
				html = '<table>'
					+ '<tr><th width="100">Address：</th><td>' + data.subTitle + '</td></tr>'
  ...
  //              this.vDemAltReq.push(this.ajaxElevation);
	...
	getElevationRusult : function (data, dataSrc)
	{
		...
		var outPutHeightSrc = "（" + "Data Source：" + GLOBE.MAP.vDemAltSRC + "）";
...
GSI.LayerTreeDialog = GSI.Dialog.extend( {
	...
	{
		alert( 'Could not load layer configuration file.' );
	},
...
GSI.ViewListDialog = GSI.Dialog.extend( {
	...
	_onOpacityBtnClick : function(li)
	{
		...
		if ( !this._opacityWindow )
		{
			...
			this._opacityValue = $( '<div>' ).addClass( 'value' ).html( 'trans:' );
		...
		this._opacityValue.html('trans:' + opacityPercentage + '%');
		...
		this._opacitySlider.off("slide").on( "slide", MA.bind(function( event, ui ) {
			...
			this._opacityValue.html('trans:' + value + '%');
...
GSI.LayersJSON = MA.Class.extend( {
	...
	_onLoadError : function()
	{
		if ( this._loadingData )
			...
			alert( 'Failed to load the layer configuration file.' );
	},
...
GSI.MapLayerList = MA.Class.extend( {
	...
	_showLoading : function(title)
	{
		if ( !this._showLoadingInc || this._showLoadingInc == 0 )
		{
			...
			GSI.Modal.LoadingMessage.show( 'loading...' );
		}
...
GLOBE.DIALOG.FOOTER = $.extend({}, new GLOBE.CLASS.DIALOG('gsi_dialog_footer'), {
	...
	{
		this.createDialog();
		this.setDialogHeader('Information at marker');
		var h = $(window).height();
    ...
    var ellipsoid = GLOBE.MAP.viewer.scene.globe.ellipsoid;
			...
			this.label1 = $('<div><span style="color:#ccc;">Address：</span>　'+address+'</div>')
				.appendTo(this.frame);
			this.label1 = $("<div>　　　　　　　　(Not entirely accurate.)</div>")
  			.css({
				'color': '#ccc',
				'font-size': '7.5pt'
			})
		...
		var dms = GSI.Utils.latLngToDMS( center );
			var lon2=(center.lat < 0 ? '-' : '') + dms.lat.d + '°' + dms.lat.m + '′' + ( Math.round( dms.lat.s * 100 ) / 100 ).toFixed(2)  + '″'
			var lat2=(center.lng < 0 ? '-' : '') + dms.lng.d + '°' + dms.lng.m + '′' + ( Math.round( dms.lng.s * 100 ) / 100 ).toFixed(2)  + '″'
		...
		this.lonlat = $('<div>' + lat + ',' + lon+'</div>')
			.appendTo(this.frame);
		this.UTM = $('<div><span style="color:#ccc;">MGRS：</span>'+utmPoint+'</div>')
			.appendTo(this.frame);
		this.a = $('<a href="http://maps.gsi.go.jp/help/howtouse.html" target="_blank"> details</a>')
		    .css({
				'color': '#333',
		...
		this.outPutHeight = $('<div><span style="color:#ccc;">Elevation：</span><span id="DemHeight">---</span></div>')
			.appendTo(this.frame);
