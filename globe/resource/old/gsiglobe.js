...
CONFIG.layerBaseFolder    = "Basemap";
...
GSI.ViewListDialog = GSI.Dialog.extend( {
...

	{
        ...
        // 透過
		var opacityBtn =$("<a>").addClass("opacity_btn").attr( {"href":"javascript:void(0);"} ).html('trans');
...
GLOBE.DIALOG.FOOTER = $.extend({}, new GLOBE.CLASS.DIALOG('gsi_dialog_footer'), {
	...
	{
		this.createDialog();
		this.setDialogHeader('Location information');
		var h = $(window).height();
    ...
    var ellipsoid = GLOBE.MAP.viewer.scene.globe.ellipsoid;
			...
			this.label1 = $('<div><span style="color:#ccc;">Address：</span>　'+address+'</div>')
				.appendTo(this.frame);
			this.label1 = $('<div>　　　　(at cross-hair position. It's not necessarily accurate.)</div>')
  			.css({
				'color': '#ccc',
				'font-size': '7.5pt'
			})
		...
		this.lonlat = $('<div>' + lat + ',' + lon+'</div>')
			.appendTo(this.frame);
		this.UTM = $('<div><span style="color:#ccc;">UTM Point：</span>'+utmPoint+'</div>')
			.appendTo(this.frame);
